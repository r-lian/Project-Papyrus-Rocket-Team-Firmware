...
int main(void) {
  ...
  PAPYRUS_LOG_INFO("Version: %d.%d.%d Build: %d", PAPYRUS_VERSION_MAJOR,
                   PAPYRUS_VERSION_MINOR, PAPYRUS_VERSION_PATCH,
                   PAPYRUS_VERSION_BUILD);
...
static papyrus_status_t create_rtos_objects(void) {
  ...
  /* Create timers */
  timer_heartbeat =
      xTimerCreate("Heartbeat", MS_TO_TICKS(1000 / HEARTBEAT_RATE_HZ), pdTRUE,
                   NULL, heartbeat_timer_callback);

  timer_watchdog_feed =
      xTimerCreate("WatchdogFeed", MS_TO_TICKS(WATCHDOG_TIMEOUT_MS / 4), pdTRUE,
                   NULL, watchdog_feed_timer_callback);
...
static papyrus_status_t create_tasks(void) {
  ...
  result = xTaskCreate(emergency_task, "Emergency", STACK_SIZE_EMERGENCY, NULL,
                       TASK_PRIORITY_EMERGENCY, &h_emergency_task);
  if (result != pdPASS)
    return PAPYRUS_ERROR_NO_MEMORY;

  /* CAN Manager Task */
  result = xTaskCreate(can_manager_task, "CANManager", STACK_SIZE_CAN_HANDLER,
                       NULL, TASK_PRIORITY_CAN_RX, &h_can_manager_task);
  if (result != pdPASS)
    return PAPYRUS_ERROR_NO_MEMORY;

  /* System Controller Task */
  result = xTaskCreate(system_controller_task, "SysCtrl", STACK_SIZE_CONTROL,
                       NULL, TASK_PRIORITY_CONTROL, &h_system_controller_task);
  if (result != pdPASS)
    return PAPYRUS_ERROR_NO_MEMORY;

  /* Data Logger Task */
  result = xTaskCreate(data_logger_task, "DataLogger", STACK_SIZE_SENSOR, NULL,
  ...
  /* Radio Handler Task */
  result =
      xTaskCreate(radio_handler_task, "RadioHandler", STACK_SIZE_CAN_HANDLER,
                  NULL, TASK_PRIORITY_CAN_TX, &h_radio_handler_task);
  if (result != pdPASS)
    return PAPYRUS_ERROR_NO_MEMORY;

  /* Power Monitor Task */
  result = xTaskCreate(power_monitor_task, "PowerMon", STACK_SIZE_SENSOR, NULL,
                       TASK_PRIORITY_SENSOR, &h_power_monitor_task);
  if (result != pdPASS)
    return PAPYRUS_ERROR_NO_MEMORY;

  /* Status Reporter Task */
  result = xTaskCreate(status_reporter_task, "StatusRpt", STACK_SIZE_STATUS,
                       NULL, TASK_PRIORITY_STATUS, &h_status_reporter_task);
  if (result != pdPASS)
    return PAPYRUS_ERROR_NO_MEMORY;
...
/**
 * @brief Emergency Handler Task - Highest priority for safety-critical
 * operations
 */
static void emergency_task(void *pvParameters) {
...
static void heartbeat_timer_callback(TimerHandle_t xTimer) {
  ...
  heartbeat_msg.id =
      can_build_id(CAN_PRIORITY_DATA, BOARD_ID_MAIN, MSG_TYPE_HEARTBEAT);
...
main_board_status_t *main_board_get_status(void) { return &g_main_status; }

main_board_config_t *main_board_get_config(void) { return &g_main_config; }

QueueHandle_t main_board_get_can_tx_queue(void) { return q_can_tx; }

QueueHandle_t main_board_get_system_event_queue(void) {
  return q_system_events;
}

QueueHandle_t main_board_get_data_log_queue(void) { return q_data_log; }
