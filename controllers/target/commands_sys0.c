...
void load_ctable_sys0(CommandRunner *runs, CommandResponder *resps,
                      CANMsgLen *lens) {
...
PapyrusStatus run_sys0_ping(CANMessage *msg, ControllerBase *controller,
  ...
  uint8_t ping_data_byte = msg->msg.short_args[0];
...
PapyrusStatus resp_sys0_ping(CANMessage *msg, CANMessage *resp,
  ...
  resp->msg.short_data[0] = ping_data_byte;
  UNUSED(err);
  return PAPYRUS_OK;
}
...
PapyrusStatus resp_sys0_query_type(CANMessage *msg, CANMessage *resp,
  ...
  resp->tHeader.Identifier =
      CAN_GENERATE_ID(controller->board_id, MSG_TYPE_RESPONSE);
  resp->tHeader.DataLength = 4;
  resp->msg.short_data[0] = controller->controller_type;
  resp->msg.short_data[1] = controller->board_revision;
  resp->msg.short_data[2] = controller->firmware_revision;
  resp->msg.short_data[3] = controller->status.num_subdevices;
  UNUSED(err);
  return PAPYRUS_OK;
}
