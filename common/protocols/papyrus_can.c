...
uint32_t can_build_id(can_priority_t priority, board_id_t source, msg_type_t msg_type)
{
    ...
    // Validate inputs
    if (priority > CAN_PRIORITY_DEBUG || 
        source > BOARD_ID_GROUND || 
...
{
    return (can_priority_t)((can_id >> CAN_ID_PRIORITY_SHIFT) & CAN_ID_PRIORITY_MASK);
}
 ...
 */
int can_message_to_string(const can_message_t* msg, char* buffer, int buffer_size)
{
    ...
    const char* msg_type_str[] = {"SYS", "CMD", "DAT", "STA", "ERR", 
                                  "CFG", "SYN", "DBG", "HB", "ACK", "NAK"};
    
    int len = snprintf(buffer, buffer_size, 
                      "ID:0x%03X [%s|%02d|%s] Len:%d Data:",
                      (unsigned int)msg->id,
                      (priority < 4) ? priority_str[priority] : "???",
                      source,
                      (msg_type < 11) ? msg_type_str[msg_type] : "???",
                      msg->length);
...
{
    switch (board_id) {
        case BOARD_ID_MAIN: return "Main Board";
        case BOARD_ID_SERVO_1: return "Servo Ctrl #1";
        case BOARD_ID_SERVO_2: return "Servo Ctrl #2";
        case BOARD_ID_SERVO_3: return "Servo Ctrl #3";
        case BOARD_ID_TC_1: return "TC Ctrl #1";
        case BOARD_ID_TC_2: return "TC Ctrl #2";
        case BOARD_ID_TC_3: return "TC Ctrl #3";
        case BOARD_ID_IO_1: return "I/O Ctrl #1";
        case BOARD_ID_IO_2: return "I/O Ctrl #2";
        case BOARD_ID_POWER: return "Power Board";
        case BOARD_ID_RADIO: return "Radio Board";
        case BOARD_ID_DEBUGGER: return "Bus Debugger";
        case BOARD_ID_GROUND: return "Ground Station";
